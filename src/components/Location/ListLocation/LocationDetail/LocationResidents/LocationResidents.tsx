...
export const LocationResidents: React.FC<{ route: IrouteLocation }> = ({
  route,
}) => {
  return (
    <>
      {route.params.location.residents.length ? (
        <Wrapper>
          <FlatList
            numColumns={2}
            ...
            ListHeaderComponent={() => <LocationTitleDetail route={route} />}
            renderItem={({ item }) => (
              <WrapperCard>
                <WrapperCharacter>
        ...
        <>
          ...
          <WrapperTextFound>
            <TeaxtFound>Not found</TeaxtFound>
          </WrapperTextFound>
        </>
      )}
    </>
