...
export const Filter: React.FC = () => {
    const [value, setValue] = useState<AppContextInterface>({ status: '', gender: '' });
    ...
    useEffect(() => {
        let filterData = data?.characters?.results?.filter((item: Result) => {
            if (value.status.length && value.gender.length) {
                return item.status == value.status && item.gender.toLowerCase() == value.gender.toLowerCase()
